...
func (v *ValueMap) AddValue(key string, value int64) (int64, error) {
	v.m.Lock()
	defer v.m.Unlock()
	if _, ok := v.Map[key]; ok {
		return -1, errors.New("key has existed")
	}
	v.Map[key] = value
	return v.Map[key], nil
}

func (v *ValueMap) UpdateValue(key string, value int64) (int64, error) {
	v.m.Lock()
	defer v.m.Unlock()
	if _, ok := v.Map[key]; !ok {
